...
#		print "typedef! [$name] as [$type]\n";
	...
	else
	{
		...
		if ($current_sentence->init_val eq "")
		{
			$new_variable->init_val(" ");
		}
		else
		{
			my $tmp_value = $current_sentence->init_val;
			$tmp_value =~ s/\t/ /g;								#/* ハードタブ排除 */
...
#			printf ("out typedef(struct) : %s %d\n", $typedef->name, $length);
	...
	printf OUT_FILE_OUT "\nFunction List\n";
	printf OUT_FILE_OUT "\tname\tret_type\tsummary\tstatic\tlines\tsteps\tcomments\tpaths\n";
	foreach $function (@functions)
	{
		printf OUT_FILE_OUT "\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n", $function->name,$function->ret_typ,$function->summary,$function->static,$function->lines,$function->steps,$function->comment,$function->paths;
	}
...
#		foreach $variable (@{$function->local_val})
		...
		&output_path($path);
		if ($footer_text ne "none")
		{
			printf OUT_PU_OUT "footer $footer_text\n";
		}
	...
	if ($footer_text ne "none")
	{
		printf OUT_FUNC_TREE "footer $footer_text\n";
	}
...
#			print "extract $1\n";
		...
		elsif ($line_text =~ /^footer_text[ \t]+([^\n]+)/)
		{
#			print "footer_text specified.\n";
			$footer_text = $1;
		}
	}
	close(SETTING_IN);
